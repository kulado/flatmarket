...
var startFlatmarketServer = require('./server').startServer
var url = require('url')
...
var OPTIONS_SCHEMA = Joi.object().keys({
    component: Joi.string().default(path.resolve(__dirname, './node_modules/gocart-web/')),
    ...
    schema: Joi.string().default(path.resolve(process.cwd(), './src/gocart.json')),
...
var LAYOUT_PATH = path.resolve(__dirname, 'layout.html')
...
module.exports = function (options) {
    ...
    var gocartServerConfig = {
        port: GOCART_SERVER_PORT,
        schemaUri: schemaUri,
    ...
    if (options.dev) {
        return Bluebird.resolve()
            ...
            .then(function () {
                return Bluebird.all([
                    startFlatmarketServer(gocartServerConfig),
