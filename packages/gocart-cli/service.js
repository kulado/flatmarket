...
var gocartSchema = require('./schema')
var Joi = require('joi')
    ...
    return function (payload) {
        return get(schemaUri, { rejectUnauthorized: false })
            .spread(function (res, schemaPayload) {
                ...
                if (!gocartSchema.isValid(schemaPayload)) return Bluebird.reject(Boom.serverTimeout(INVALID_SCHEMA_ERROR))
                if (!isValidRequest(schemaPayload, payload)) return Bluebird.reject(Boom.badRequest())
                var schema = Joi.validate(schemaPayload, gocartSchema).value
