...
var OPTIONS = {
    port: 8000,
    schemaUri: 'https://just-a-test-server.com/gocart.json',
    stripeSecretKey: 'just_a_test_key',
}

describe('gocart-server', function () {
    ...
    it('should load the plugin and start', function (done) {
        startServer(OPTIONS)
            .then(function (server) {
                expect(server.lookup('gocart')).to.have.property('path', '/')
                expect(server.lookup('gocart-preflight')).to.have.property('path', '/')
                return done()
            })
    })
